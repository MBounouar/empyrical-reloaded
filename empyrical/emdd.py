import numpy as np
from scipy import interpolate


def expected_max_drawdown(
    mu: float,
    sigma: float,
    t: float,
) -> float:
    """
    Determines the expected maximum drawdown of a brownian motion,
    given drift and diffusion.


    Parameters
    ----------
    mu : float
        The drift term of a Brownian motion with drift.
    sigma : float
        The diffusion term of a Brownian motion with drift.
    t : float
        A time period of interest in years

    Returns
    -------
    expected_max_drawdown : float

    Note
    -----
    See http://www.cs.rpi.edu/~magdon/ps/journal/drawdown_journal.pdf
    and https://doi.org/10.1239/jap/1077134674
    for more details.
    """

    sigma2 = np.power(sigma, 2)

    def emdd_q(x: float, positive: bool = True):
        """Q function based on lookup table"""

        if x < 0.0005:
            return 0.5 * np.sqrt(np.pi * x)

        if positive:
            if x > 5000:
                return 0.25 * np.log(x) + 0.49088

            XQ = np.array(
                [
                    [0.0005, 0.019690],
                    [0.0010, 0.027694],
                    [0.0015, 0.033789],
                    [0.0020, 0.038896],
                    [0.0025, 0.043372],
                    [0.0050, 0.060721],
                    [0.0075, 0.073808],
                    [0.0100, 0.084693],
                    [0.0125, 0.094171],
                    [0.0150, 0.102651],
                    [0.0175, 0.110375],
                    [0.0200, 0.117503],
                    [0.0225, 0.124142],
                    [0.0250, 0.130374],
                    [0.0275, 0.136259],
                    [0.0300, 0.141842],
                    [0.0325, 0.147162],
                    [0.0350, 0.152249],
                    [0.0375, 0.157127],
                    [0.0400, 0.161817],
                    [0.0425, 0.166337],
                    [0.0450, 0.170702],
                    [0.0475, 0.174924],
                    [0.0500, 0.179015],
                    [0.0550, 0.186842],
                    [0.0600, 0.194248],
                    [0.0650, 0.201287],
                    [0.0700, 0.207999],
                    [0.0750, 0.214421],
                    [0.0800, 0.220581],
                    [0.0850, 0.226505],
                    [0.0900, 0.232212],
                    [0.0950, 0.237722],
                    [0.1000, 0.243050],
                    [0.1500, 0.288719],
                    [0.2000, 0.325071],
                    [0.2500, 0.355581],
                    [0.3000, 0.382016],
                    [0.3500, 0.405415],
                    [0.4000, 0.426452],
                    [0.4500, 0.445588],
                    [0.5000, 0.463159],
                    [1.0000, 0.588642],
                    [1.5000, 0.668992],
                    [2.0000, 0.728540],
                    [2.5000, 0.775976],
                    [3.0000, 0.815456],
                    [3.5000, 0.849298],
                    [4.0000, 0.878933],
                    [4.5000, 0.905305],
                    [5.0000, 0.929070],
                    [10.000, 1.088998],
                    [15.000, 1.184918],
                    [20.000, 1.253794],
                    [25.000, 1.307607],
                    [30.000, 1.351794],
                    [35.000, 1.389289],
                    [40.000, 1.421860],
                    [45.000, 1.450654],
                    [50.000, 1.476457],
                    [100.00, 1.647113],
                    [150.00, 1.747485],
                    [200.00, 1.818873],
                    [250.00, 1.874323],
                    [300.00, 1.919671],
                    [350.00, 1.958037],
                    [400.00, 1.991288],
                    [450.00, 2.020630],
                    [1000.0, 2.219765],
                    [2000.0, 2.392826],
                    [3000.0, 2.494109],
                    [4000.0, 2.565985],
                    [5000.0, 2.621743],
                ]
            )
        else:  # negative
            if x > 5:
                return x + 0.5

            XQ = np.array(
                [
                    [0.0005, 0.019965],
                    [0.0010, 0.028394],
                    [0.0015, 0.034874],
                    [0.0020, 0.040369],
                    [0.0025, 0.045256],
                    [0.0050, 0.064633],
                    [0.0075, 0.079746],
                    [0.0100, 0.092708],
                    [0.0125, 0.104259],
                    [0.0150, 0.114814],
                    [0.0175, 0.124608],
                    [0.0200, 0.133772],
                    [0.0225, 0.142429],
                    [0.0250, 0.150739],
                    [0.0275, 0.158565],
                    [0.0300, 0.166229],
                    [0.0325, 0.173756],
                    [0.0350, 0.180793],
                    [0.0375, 0.187739],
                    [0.0400, 0.194489],
                    [0.0425, 0.201094],
                    [0.0450, 0.207572],
                    [0.0475, 0.213877],
                    [0.0500, 0.220056],
                    [0.0550, 0.231797],
                    [0.0600, 0.243374],
                    [0.0650, 0.254585],
                    [0.0700, 0.265472],
                    [0.0750, 0.276070],
                    [0.0800, 0.286406],
                    [0.0850, 0.296507],
                    [0.0900, 0.306393],
                    [0.0950, 0.316066],
                    [0.1000, 0.325586],
                    [0.1500, 0.413136],
                    [0.2000, 0.491599],
                    [0.2500, 0.564333],
                    [0.3000, 0.633007],
                    [0.3500, 0.698849],
                    [0.4000, 0.762455],
                    [0.5000, 0.884593],
                    [1.0000, 1.445520],
                    [1.5000, 1.970740],
                    [2.0000, 2.483960],
                    [2.5000, 2.990940],
                    [3.0000, 3.492520],
                    [3.5000, 3.995190],
                    [4.0000, 4.492380],
                    [4.5000, 4.990430],
                    [5.0000, 5.498820],
                ]
            )

        return interpolate.interp1d(np.log(XQ[:, 0]), XQ[:, 1], kind="cubic")(
            [np.log(x)]
        )[0]

    if (not np.isfinite(mu)) | (not np.isfinite(sigma)) | (sigma <= 0):
        return np.nan

    if mu == 0:
        return np.sqrt(np.pi / 2) * sigma * np.sqrt(t)

    alpha = (2 * sigma2) / mu
    x = (np.power(mu, 2) * t) / (2 * sigma2)
    Q = emdd_q(x, positive=True if mu > 0 else False)
    return np.sign(mu) * alpha * Q


if __name__ == "__main__":

    print(expected_max_drawdown(mu=0, sigma=1, t=1, gbm=False))
    print(expected_max_drawdown(mu=0, sigma=1, t=1, gbm=True))
    print(
        expected_max_drawdown(mu=0.1004, sigma=0.1548, t=24.25, gbm=False),
        0.4456,
    )

    print(
        expected_max_drawdown(mu=0.0701, sigma=0.1666, t=19.83, gbm=False),
        0.5554,
    )
    print(
        expected_max_drawdown(mu=0.1120, sigma=0.2438, t=19.42, gbm=False),
        0.7787,
    )
    print(
        expected_max_drawdown(mu=0.1565, sigma=0.0578, t=3.08, gbm=False),
        0.0477,
    )
    # print(
    #     expected_max_drawdown(
    #         mu=np.array([0]),
    #         sigma=np.array([1]),
    #         t=1000,
    #         gbm=False,
    #     )
    # )
